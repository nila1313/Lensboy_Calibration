...
def load_poses(path):
    with open(path, "r") as f:
        records = json.load(f)
    return {int(r["frame_index"]): r for r in records}
...
def average_transform(candidates):
    ...
    T_avg[:3, :3] = R_avg
    T_avg[:3, 3] = t_avg
    return T_avg, q_avg, t_avg


def summarize_candidates(candidates, common_frames_before, removed_rotation_outliers):
    T_avg, q_avg, t_avg = average_transform(candidates)
    ...
    for c in candidates:
        R_c = c["T_ref_cam"][:3, :3]
        t_c = c["T_ref_cam"][:3, 3]
        rot_errors.append(rotation_error_deg(R_c, T_avg[:3, :3]))
        trans_errors.append(float(np.linalg.norm(t_c - t_avg)))

    return {
        "num_common_frames_before_filtering": common_frames_before,
        "num_candidates_after_filtering": len(candidates),
        "num_removed_rotation_outliers": removed_rotation_outliers,
...
def estimate_relative_transform(ref_records, cam_records, max_pose_rms, max_rotation_outlier_deg):
    ...
    if len(candidates) >= 4:
        T_initial, _, _ = average_transform(candidates)
        kept = []
        removed = []

        for c in candidates:
            err = rotation_error_deg(c["T_ref_cam"][:3, :3], T_initial[:3, :3])
            if err <= max_rotation_outlier_deg:
                kept.append(c)
            else:
                removed.append({
    ...
    result["rotation_outliers_removed"] = removed_list
    return result
...
def plot_camera_rig(results, reference_camera, output_path):
    ...
    for cam, result in results.items():
        if cam == reference_camera or result["status"] != "success":
            continue
        T_ref_cam = np.asarray(result["T_ref_cam"], dtype=np.float64)
        positions[cam] = T_ref_cam[:3, 3]
    ...
    for cam, pos in positions.items():
        if cam != reference_camera:
            ax.plot([ref_pos[0], pos[0]], [ref_pos[1], pos[1]], [ref_pos[2], pos[2]])

    ax.set_title(f"Robust estimated camera rig in {reference_camera} coordinates")
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_zlabel("z")
...
def main():
    ...
    parser.add_argument("--max-pose-rms", type=float, default=2.0)
    ...
    if ref_cam not in all_poses:
        raise RuntimeError(f"Reference camera {ref_cam} has no poses.")

    ref_records = all_poses[ref_cam]
    results = {}

    print("=" * 80)
    print("ROBUST DARK MULTICAMERA RELATIVE POSE ESTIMATION")
    print("=" * 80)
    ...
    results[ref_cam] = {
        ...
        "num_candidates_after_filtering": len(ref_records),
    }
    ...
    for cam in cameras:
        ...
        result = estimate_relative_transform(
            ...
            max_rotation_outlier_deg=args.max_rotation_outlier_deg,
        )
        ...
        if result is None:
            ...
            print(f"{ref_cam} <- {cam}: no valid candidates")
            continue
        ...
        print(f"Candidates after filtering: {result['num_candidates_after_filtering']}")
        print(f"Rotation outliers removed: {result['num_removed_rotation_outliers']}")
    ...
    output_path = reports_folder / f"robust_relative_camera_poses_reference_{ref_cam}.json"
    with open(output_path, "w") as f:
        json.dump(results, f, indent=4)

    final_path = final_folder / f"final_robust_relative_camera_poses_reference_{ref_cam}.json"
    with open(final_path, "w") as f:
        json.dump(results, f, indent=4)

    plot_path = plots_folder / f"robust_camera_rig_reference_{ref_cam}.png"
    ...
    print("DONE")
    print("=" * 80)
    print(f"Robust relative poses saved to: {output_path}")
    print(f"Final robust copy saved to: {final_path}")
    print(f"Robust rig plot saved to: {plot_path}")
